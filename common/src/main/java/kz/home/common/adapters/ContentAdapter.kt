...
class ContentAdapter<T : ContentType>(
    ...
    var collection: List<BaseContentItem<T>>
            by Delegates.observable(emptyList()) { _, old, new ->
                DiffUtil.calculateDiff(BaseContentItem.Diff(old, new))
                        .dispatchUpdatesTo(this@ContentAdapter)
                afterDispatchList()
            }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int) =
            typeSet.first { it.type() == viewType }.let { type ->
                LayoutInflater.from(parent.context).
                inflate(type.getLayout(), parent, false).let { view ->
                    (type.createHolder(view) as BaseHolder<T, BaseContentItem<T>>).apply {
